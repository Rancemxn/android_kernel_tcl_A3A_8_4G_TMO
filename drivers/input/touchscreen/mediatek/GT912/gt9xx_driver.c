...
s32 i2c_dma_read(struct i2c_client *client, u16 addr, u8 *rxbuf, s32 len)
{
    ...
    GTP_ERROR("Dma I2C Read Error: 0x%04X, %d byte(s), err-code: %d", addr, len,
              ret);
    return ret;
}
...
s32 i2c_dma_write(struct i2c_client *client, u16 addr, u8 *txbuf, s32 len)
{
    ...
    GTP_ERROR("Dma I2C Write Error: 0x%04X, %d byte(s), err-code: %d", addr, len,
              ret);
    return ret;
}
...
s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    gtp_cfg_version = send_cfg_buf[sensor_id][0];
    GTP_INFO("CTP_CONFIG_GROUP%d used, config length: %d", sensor_id, cfg_len);
    if (cfg_len < GTP_CONFIG_MIN_LENGTH) {
        GTP_ERROR("CTP_CONFIG_GROUP%d is INVALID CONFIG GROUP! NO Config Sent!",
                  sensor_id);
...
#else
    {
        ...
        if (ret == SUCCESS)
        {
            ...
            sensor_id, send_cfg_buf[sensor_id][0], send_cfg_buf[sensor_id][0], opr_buf[0],
            opr_buf[0]);
    ...
    if (CHIP_TYPE_GT9F == gtp_chip_type) {
        u8 have_key = 0;
        driver_num = (config[CFG_LOC_DRVA_NUM] & 0x1F) + (config[CFG_LOC_DRVB_NUM] &
                     0x1F);
        sensor_num = (config[CFG_LOC_SENS_NUM] & 0x0F) + ((config[CFG_LOC_SENS_NUM] >>
                     4) & 0x0F);
...
reset_proc:
    ...
    GTP_INFO("turn on power!\n");
    ret = regulator_enable(tpd->reg);
    if (ret != 0)
        GTP_ERROR("Failed to enable vdd: %d\n", ret);
    gtp_reset_guitar(client, 20);
#ifdef CONFIG_GTP_COMPATIBLE_MODE
...
static u8 gtp_bak_ref_proc(struct i2c_client *client, u8 mode)
{
    ...
    GTP_INFO("[gtp_bak_ref_proc]Waiting for FS %d", gtp_ref_retries);
    ...
    /* Always write default ref */
    gtp_ref_retries = GTP_CHK_FS_MNT_MAX;
    if (gtp_ref_retries >= GTP_CHK_FS_MNT_MAX) {
    ...
    if (GTP_BAK_REF_STORE == mode) {
        ...
        for (j = 0; j < ref_grps; ++j) {
            ...
            for (i = 0; i < ref_seg_len - 2; i += 2)
                ref_chksum += ((refp[i + j * ref_seg_len] << 8) + refp[i + 1 + j *
                               ref_seg_len]);
...
static u8 gtp_main_clk_proc(struct i2c_client *client)
{
    ...
    if (SUCCESS == ret)
        goto send_main_clk;
    GTP_INFO("[gtp_main_clk_proc]Waiting for FS %d", gtp_clk_retries);
    if (gup_check_fs_mounted("/data") == FAIL) {
        ...
        if (!IS_ERR(flp)) {
            ...
            if (ret > 0) {
                ret = gtp_check_clk_legality();
                if (SUCCESS == ret) {
                    GTP_INFO("[gtp_main_clk_proc]Open & read & check clk file success.");
                    goto send_main_clk;
                }
            }
        }
                         ...
                         const struct i2c_device_id *id)
{
    ...
    gtp_fw_version = version_info;

    ret = gtp_init_panel(client);
    if (ret < 0)
        GTP_ERROR("GTP init panel failed.");
    GTP_INFO("gtp_init_panel success");
...
void force_reset_guitar(void)
{
    ...
    if (ret != 0)
        GTP_ERROR("Failed to disable reg-vgp6: %d\n", ret);
    msleep(30);
    /* Power on TP */
    ret = regulator_enable(tpd->reg);
    if (ret != 0)
        GTP_ERROR("Failed to enable reg-vgp6: %d\n", ret);
    msleep(30);
    for (i = 0; i < 5; i++) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (ret < 0) {
            ...
            if ((esd_buf[0] == 0xAA) || (esd_buf[1] != 0xAA)) {
                ...
                GTP_INFO("[Check]0x8040 = 0x%02X, 0x8041 = 0x%02X", chk_buf[0], chk_buf[1]);
...
static int touch_event_handler(void *unused)
{
    ...
    do {
        ...
        mutex_lock(&i2c_access);
#ifdef CONFIG_GTP_GESTURE_WAKEUP
        if ((gesture_data.enabled || gesture_wakeup_flag) && tpd_halt == 1) {
...
static int tpd_local_init(void)
{
    ...
    if (retval != 0) {
        GTP_ERROR("Failed to set vdd voltage: %d\n", retval);
        return -1;
    }
    ...
    input_set_abs_params(tpd->dev, ABS_MT_TRACKING_ID, 0, (GTP_MAX_TOUCH - 1), 0,
                         0);
    if (tpd_dts_data.use_tpd_button) {
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
    ...
    if (CHIP_TYPE_GT9F == gtp_chip_type) {
        u8 opr_buf[2] = {0};
        while (retry++ < 10) {
            if (gesture_data.enabled || gesture_wakeup_flag) {
                ...
                //disable_irq(touch_irq);
                gtp_reset_guitar(client, 20);
                ret = gup_reload_fw_dsp(client, GTP_FL_READ_REPAIR);
                //enable_irq(touch_irq);
                ...
                GTP_INFO("gesture wakeup sleep success!");
